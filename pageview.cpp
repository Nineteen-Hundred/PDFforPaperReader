...
qreal PageView::zoom() const
{
    return m_zoom;
}

void PageView::keyPressEvent(QKeyEvent *ev)
{
...
void PageView::keyReleaseEvent(QKeyEvent *ev)
{
    this->ctrl_flag = false;
}

void PageView::wheelEvent(QWheelEvent *ev)
{
    if(this->ctrl_flag)
    {
        if(ev->delta()>0)
        {
            ...
            {
                zoom_value*=1.25;
                emit this->setZoom(this->zoom_value);
            }
        }
        else
        {
    ...
    else
    {
        if(ev->delta()<0)
        {
            this->bar->setValue(this->bar->value()+80);
        }
        else
        {
            this->bar->setValue(this->bar->value()-80);
